...
export type FormField =
  ...
  | { key: string; label: string; type: 'color'; required?: boolean };
...
export function SidePopupForm({
  ...
  useEffect(() => {
    ...
    fields.forEach((field) => {
      ...
      else if (field.type === 'color') initialData[field.key] = '#808080';
    ...
    <>
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <form
            id="side-popup-form"
            className="flex-1 flex flex-col relative p-6 space-y-6 overflow-y-auto"
              ...
              <div key={field.key} className="space-y-2">
                ...
                {field.type === 'color' && (
                  <input
                    type="color"
                    id={field.key}
                    value={typeof formData[field.key] === 'string' ? String(formData[field.key]) : '#808080'}
                    onChange={(e) => handleChange(field.key, e.target.value)}
                    className="h-10 w-16 border rounded p-0 cursor-pointer"
                  />
                )}
