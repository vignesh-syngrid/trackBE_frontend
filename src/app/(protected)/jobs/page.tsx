...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (typeof window !== 'undefined') {
          try {
            sessionStorage.setItem('job_status_opts', JSON.stringify(opts));
          } catch {}
        }
      } catch {
        /* ignore */
  ...
  const buildServerParams = useCallback((): Record<string, string | number> => {
    ...
    if (appliedIds.assignee_id)
      (params as any).assignee_id = appliedIds.assignee_id;
  ...
  const onApplyFilters = () => {
    ...
    setAppliedIds({
      client_id: uiClientId || undefined,
      assignee_id: uiAssigneeId || undefined,
      region_id: uiRegionId || undefined,
    });
  ...
  const JobActions = (j: JobRow) => (
    <div className="flex gap-2 justify-center">
      <Button
        ...
        onClick={() => router.push(`/jobs/chat/${j.job_id}`)}
      >
      ...
      {canEdit &&
        (() => {
          ...
          const locked = ['completed', 'rejected', 'unresolved'].includes(
            statusKey
          );
          const btn = (
            <Button
          ...
          return locked ? (
            <span
              ...
              title="Editing disabled for completed/rejected/unresolved jobs"
            >
              {btn}
            </span>
          ) : (
      ...
      <Card className="pt-0">
        ...
        <div className="flex flex-wrap gap-2 bg-[#f9fafb] items-center p-4 rounded-t-xl">
          ...
          <AsyncSearchSelect
            ...
            value={uiClientText}
            onChange={(label, opt) => {
              setUiClientText(label);
              setUiClientId(opt?.value || '');
            }}
            mapResponse={(d: any) =>
              (d?.data || []).map((c: any) => ({
                value: c.client_id,
          ...
          <AsyncSearchSelect
            ...
            onChange={(label, opt) => {
              setUiAssigneeText(label);
              setUiAssigneeId(opt?.value || '');
            }}
            mapResponse={(d: any) =>
              (d?.data || []).map((u: any) => ({
                value: u.user_id,
          ...
          <AsyncSearchSelect
            ...
            value={uiRegionText}
            onChange={(label, opt) => {
              setUiRegionText(label);
              setUiRegionId(opt?.value || '');
            }}
            mapResponse={(d: any) =>
              (d?.data || []).map((r: any) => ({
                value: r.region_id,
          ...
          <Table>
            ...
            <TableBody>
              ...
              {filteredRows.map((j) => {
                ...
                const lastUpdated = fmtDateTime12(
                  j.updatedAt ?? (j as any).updated_at
                );
